---
name: compranet-search
description: Busca licitaciones y contrataciones públicas de México (CompraNet / Contrataciones Abiertas).
license: MIT
metadata:
  category: business
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct compranet-search
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files compranet-search
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/compranet-search/instruction.md.
