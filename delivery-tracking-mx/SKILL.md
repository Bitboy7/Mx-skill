---
name: delivery-tracking-mx
description: Seguimiento de paquetes en México: Estafeta (endpoint público verificado).
license: MIT
metadata:
  category: shopping
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct delivery-tracking-mx
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files delivery-tracking-mx
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/delivery-tracking-mx/instruction.md.
