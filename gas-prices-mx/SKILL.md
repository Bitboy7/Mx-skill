---
name: gas-prices-mx
description: Encuentra las gasolineras más baratas cerca de una ubicación en México usando la API pública de precios de la CRE (datos.gob.mx).
license: MIT
metadata:
  category: transport
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct gas-prices-mx
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files gas-prices-mx
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/gas-prices-mx/instruction.md.
