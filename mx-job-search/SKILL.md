---
name: mx-job-search
description: Busca vacantes de empleo en México/LATAM (tecnología e IA) con la API pública Vacantes Digitales, sin API key.
license: MIT
metadata:
  category: jobs
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct mx-job-search
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files mx-job-search
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/mx-job-search/instruction.md.
