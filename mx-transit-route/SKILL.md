---
name: mx-transit-route
description: Rutas de transporte en México (caminando/auto/bici) con OSRM público y geocodificación Open-Meteo.
license: MIT
metadata:
  category: transport
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct mx-transit-route
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files mx-transit-route
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/mx-transit-route/instruction.md.
