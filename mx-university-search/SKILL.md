---
name: mx-university-search
description: Busca universidades de México (públicas y privadas) y sus carreras, con enlaces oficiales.
license: MIT
metadata:
  category: education
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct mx-university-search
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files mx-university-search
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/mx-university-search/instruction.md.
