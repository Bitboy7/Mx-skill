---
name: sat-rfc-lookup
description: Analiza y valida la estructura del RFC (SAT México): persona física/moral, fecha real y desglose.
license: MIT
metadata:
  category: finance
  locale: es-MX
...
```bash
npx -y @nomadamas/k-skill@0 instruct sat-rfc-lookup
```

The CLI detects the current runtime (Dolshoi vault/CloakBrowser vs generic) and prints only the applicable instructions, always up to date. Helper files bundled with the CLI are listed by:

```bash
npx -y @nomadamas/k-skill@0 files sat-rfc-lookup
```

If `npx` is unavailable, install Node.js 18+ or follow https://github.com/NomaDamas/k-skill#readme, or read the source instructions at https://github.com/NomaDamas/k-skill/blob/main/sat-rfc-lookup/instruction.md.
